...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            outputs = model(inputs)
            _, predicted = torch.max(outputs, 1)
            # Store the true labels and predicted labels for evaluation
            all_labels.extend(labels.cpu().numpy())
            all_predictions.extend(predicted.cpu().numpy())
        # Calculate and print the classification report
        print(f"Test Accuracy: {metrics.accuracy_score(all_labels, all_predictions):.4f}")
        print(f"Test Classification Report:")
        print(metrics.classification_report(all_labels, all_predictions, target_names=dataset.genres))
